...
* `resource/turbot_policy_pack_attachment`: Fixed attachments being silently dropped when several policy packs are attached to the same resource in one apply. `attachSmartFolders` is read-modify-write on the target's attachment list, so concurrent calls for one target lost updates — attaching six packs to one folder reported success for all six and persisted four, with no error, leaving the difference to surface as drift on a later plan. Attachment writes are now serialised per target resource, and a write that does not persist fails loudly instead of being reported as successful. Detaches are verified too: a lost detach previously had no backstop at all, leaving a pack attached and still evaluating after Terraform had dropped it from state. ([#246](https://github.com/turbot/terraform-provider-turbot/issues/246))
* `resource/turbot_smart_folder_attachment`: Same fix applied. ([#246](https://github.com/turbot/terraform-provider-turbot/issues/246))
* `resource/turbot_policy_pack_attachment`, `resource/turbot_smart_folder_attachment`: `Exists` no longer decides whether an attachment has left the workspace by matching `not found` in error text. `errors.NotFoundError` matches that phrase anywhere, so an unrelated not-found could drop a live attachment out of state and have Terraform recreate it. The attachment read now asks the API to return null for a missing target (`notFound: RETURN_NULL`) and the decision is made from the response shape. ([#246](https://github.com/turbot/terraform-provider-turbot/issues/246))
* `provider`: GraphQL requests now carry a deadline. Previously `doRequest` used `context.Background()` with no timeout, so a hung connection hung the whole apply indefinitely — and after the per-target attachment serialisation above, a hung call also held that target's lock and stalled every sibling write. Requests default to a 15-minute deadline, configurable via the new `request_timeout` provider argument (a Go duration string such as `"30s"` or `"10m"`). ([#244](https://github.com/turbot/terraform-provider-turbot/issues/244))
* `errors`: `NotFoundError` no longer matches the substring `not found` anywhere in an error. It matched `(?i)not found` against the whole message, so an unrelated error that merely mentioned a missing dependency (for example `"policy type not found"`, which the provider itself generates) could be read as "the resource is gone" and drop a live resource from state. It now matches only the shapes Guardrails actually emits for a genuinely missing resource — the structured `Not Found:` prefix and the provider's own `resource not found:` wrapping — a strictly narrower match that cannot introduce new false negatives. ([#246](https://github.com/turbot/terraform-provider-turbot/issues/246))

SECURITY:

* All twelve read/delete query builders that took a caller-supplied identifier now pass it as a GraphQL variable instead of interpolating it into the query document with `fmt.Sprintf`. An identifier containing a double quote could previously escape the string literal and append arbitrary aliased GraphQL, executed with the provider's credentials — confirmed exploitable against a live workspace, where an injected field returned an unrelated resource's data into Terraform state. The identifiers are config- and import-reachable. Affected builders: `readResourceQuery`, `readFullResourceQuery`, `getResourceTypeIdQuery`, `readSmartFolderQuery`, `deleteSmartFolderMutation`, `readModQuery`, `readGrantQuery`, `readActiveGrantQuery`, `readWatchQuery`, `deleteWatchMutation`, `readPolicySettingQuery`, `readGoogleDirectoryQuery`. Each field's `id` argument type was confirmed by schema introspection before conversion. ([#244](https://github.com/turbot/terraform-provider-turbot/issues/244))
* The policy pack read queries introduced above pass their identifier as a GraphQL variable rather than interpolating it into the query document, matching what every mutation in this client already does. Interpolation would allow a config-supplied identifier containing a double quote to escape the string literal and append arbitrary GraphQL, executed with the provider's credentials. ([#244](https://github.com/turbot/terraform-provider-turbot/issues/244))
* CI no longer injects live Guardrails workspace credentials into the unit-test matrix, which never used them — acceptance tests self-skip without `TF_ACC=1`. ([#244](https://github.com/turbot/terraform-provider-turbot/issues/244))

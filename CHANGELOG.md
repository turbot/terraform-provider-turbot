...
* `provider`: GraphQL requests now carry a deadline. Previously `doRequest` used `context.Background()` with no timeout, so a hung connection hung the whole apply indefinitely — and after the per-target attachment serialisation added in #246, a hung call also held that target's lock and stalled every sibling write to the same resource. Requests default to a 15-minute deadline (deliberately generous, since Guardrails harvest and control operations can be legitimately slow), configurable via the new `request_timeout` provider argument (a Go duration string such as `"30s"` or `"10m"`). ([#250](https://github.com/turbot/terraform-provider-turbot/issues/250))
* `errors`: `NotFoundError` no longer matches the substring `not found` anywhere in an error. It matched `(?i)not found` against the whole message, so an unrelated error that merely mentioned a missing dependency (for example `"policy type not found"`, which the provider itself generates in the policy setting resource) could be read as "the resource is gone" and drop a live resource from Terraform state. It now matches only the shapes Guardrails actually emits for a genuinely missing resource — the structured `Not Found:` prefix and the provider's own `resource not found:` wrapping — a strictly narrower match that cannot introduce new false negatives. ([#250](https://github.com/turbot/terraform-provider-turbot/issues/250))

SECURITY:

* Hardened GraphQL query construction: the twelve read/delete query builders that took a caller-supplied identifier now pass it as a GraphQL variable (`$id: ID!`) instead of interpolating it into the query document with `fmt.Sprintf`, matching what every mutation in this client already does. This prevents a crafted identifier from altering the structure of the query. Each field's `id` argument type was confirmed by schema introspection before conversion. ([#250](https://github.com/turbot/terraform-provider-turbot/issues/250))
